...
export default async function JioPlusPage() {
    ...
    return (
        <main className="h-screen mt-20 p-8">
            <h1 className="text-2xl font-medium mb-6"><span className="text-yellow-400">Watchzy+</span> Premium Videos</h1>
            <ul
                className={cn("flex gap-4 w-full overflow-scroll no-scrollbar p-4")}
            >
                {videos?.map((video, index) => (
                    <Link
                        key={index}
                        href={`watchzy+/watch?id=${video.id}`}
